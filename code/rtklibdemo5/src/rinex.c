...
static const char obscodes[]="CLDSPRI";    /* obs type codes */
                          ...
                          sigind_t *index, obsd_t *obs)
{
    ...
    for (i=0;i<ind->n;i++) {
        ...
        switch (ind->type[i]) {
            ...
            case 4: if (val[i] == 0) val[i] = val[i] + 1; 
                    obs->PLD[p[i]] = val[i];
                    break;
            case 5: obs->R_V[p[i]] = val[i];                           break;
            case 6: obs->C_V[p[i]] = val[i];                           break;
        }
    }
...
static int addobsdata(obs_t *obs, const obsd_t *data)
{
    ...
    if (obs->nmax<=obs->n) {
        if (obs->nmax <= 0) obs->nmax = NINCOBS; else obs->nmax += 100000;
                      ...
                      char tobs[][MAXOBSTYPE][4], obs_t *obs, sta_t *sta)
{
    ...
    memset(data, 0, sizeof(obsd_t) * MAXOBS);
    /* read rinex obs data body */
