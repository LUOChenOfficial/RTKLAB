...
const prcopt_t prcopt_default={ /* defaults processing options */
    ...
    {100.0,0.003,0.003,0.0,1.0,55.0}, /* err[] */
    {30.0,0.03,0.3},            /* std[] */
    {1E-4,1E-3,1E-4,1E-1,1E-2,0.0}, /* prn[] */
    5E-12,                      /* sclkstab */
    {3.0,0.25,0.0,1E-9,1E-5,0.0,0.0,0.0}, /* thresar */
    15.0 * D2R,0.0,0.05,0.001,0.01,      /* elmaskar,elmaskhold,thresslip,varholdamb,gainholdamb */
...
/* get matrix trace------------------求矩阵的秩-PH-20211229*/
extern double getmattr(double* A, int n)
{
    double tr = 0.0;
    for (int i = 0; i < n; i++) {
        double a = A[i + i * n];
...
extern int Helmertest(int nv, int ns,
    double* Q, double* v, double* H, double* dx, sol_t* sol) {
    int nn = (nv - ns), i = 0, j = 0;
    //统计GPS/BDS/GLO卫星数 (在rescode中已进行了统计卫星数，这里直接赋值)       
    int GAL_NUM = sol->GAL_NUM, GPS_NUM = sol->GPS_NUM, GLO_NUM = sol->GLO_NUM;
    //构造矩阵 
    double Q_H1[NX * NX], Q_H2[NX * NX], Q_H3[NX * NX], S[9], W[3], delta[3], del1, del2, del3;
    double QN1[NX * NX], QN1_2[NX * NX], QN2[NX * NX], QN2_2[NX * NX], QN3[NX * NX], QN3_2[NX * NX], QN1QN2[NX * NX], QN1QN3[NX * NX], QN2QN3[NX * NX];
    double* B1, * B2, * B3, * v1, * v2, * v3;
    v1 = zeros(GPS_NUM, 1), v2 = zeros(GLO_NUM, 1), v3 = zeros(GAL_NUM + nn, 1);
    B1 = zeros(GPS_NUM, NX), B2 = zeros(GLO_NUM, NX), B3 = zeros(GAL_NUM + nn, NX);
    //方差分量估计
    for (int a = 0; a < 10; a++) {
        for (i = 0; i < GPS_NUM; i++) {
            for (j = 0; j < NX; j++)  B1[j + i * NX] = H[j + i * NX];
        }
        for (i = 0; i < GLO_NUM; i++) {
            for (j = 0; j < NX; j++)  B2[j + i * NX] = H[j + (i + GPS_NUM) * NX];
        }
        for (i = 0; i < GAL_NUM + nn; i++) {
            for (j = 0; j < NX; j++)  B3[j + i * NX] = H[j + (i + GPS_NUM + GLO_NUM) * NX];
        }
        matmul("TN", GPS_NUM, 1, NX, 1.0, B1, dx, 0.0, v1);  //v=BX
        matmul("TN", GLO_NUM, 1, NX, 1.0, B2, dx, 0.0, v2);
        matmul("TN", GAL_NUM + nn, 1, NX, 1.0, B3, dx, 0.0, v3);
        for (i = 0; i < GPS_NUM; i++)  v1[i] -= v[i];         //V=BX-L
        for (i = 0; i < GLO_NUM; i++)  v2[i] -= v[i + GPS_NUM];
        for (i = 0; i < GAL_NUM + nn; i++)  v3[i] -= v[i + GPS_NUM + GLO_NUM];
        W[0] = dot(v1, v1, GPS_NUM);           //VTPV
        W[1] = dot(v2, v2, GLO_NUM);
        W[2] = dot(v3, v3, GAL_NUM + nn);
        matmul("NT", NX, NX, GPS_NUM, 1.0, B1, B1, 0.0, Q_H1);//BTPB
        matmul("NT", NX, NX, GLO_NUM, 1.0, B2, B2, 0.0, Q_H2);
        matmul("NT", NX, NX, GAL_NUM + nn, 1.0, B3, B3, 0.0, Q_H3);
        matmul("TT", NX, NX, NX, 1.0, Q, Q_H1, 0.0, QN1);   //N-1Ni
        matmul("TT", NX, NX, NX, 1.0, Q, Q_H2, 0.0, QN2);
        matmul("TT", NX, NX, NX, 1.0, Q, Q_H3, 0.0, QN3);
        matmul("TT", NX, NX, NX, 1.0, QN1, QN1, 0.0, QN1_2); //N-1NiN-1Ni
        matmul("TT", NX, NX, NX, 1.0, QN2, QN2, 0.0, QN2_2);
        matmul("TT", NX, NX, NX, 1.0, QN3, QN3, 0.0, QN3_2);
        matmul("TT", NX, NX, NX, 1.0, QN1, QN2, 0.0, QN1QN2); //N-1NiN-1Nj
        matmul("TT", NX, NX, NX, 1.0, QN2, QN3, 0.0, QN2QN3);
        matmul("TT", NX, NX, NX, 1.0, QN1, QN3, 0.0, QN1QN3);
        double tr_QN1 = getmattr(QN1, NX), tr_QN1_2 = getmattr(QN1_2, NX);
        double tr_QN2 = getmattr(QN2, NX), tr_QN2_2 = getmattr(QN2_2, NX);
        double tr_QN3 = getmattr(QN3, NX), tr_QN3_2 = getmattr(QN3_2, NX);
        double tr_QN1QN2 = getmattr(QN1QN2, NX);
        double tr_QN2QN3 = getmattr(QN2QN3, NX);
        double tr_QN1QN3 = getmattr(QN1QN3, NX);
        S[0] = (double)GPS_NUM - 2 * tr_QN1 + tr_QN1_2, S[1] = tr_QN1QN2, S[2] = tr_QN1QN3;
        S[3] = S[1], S[4] = (double)GLO_NUM - 2 * tr_QN2 + tr_QN2_2, S[5] = tr_QN2QN3;
        S[6] = S[2], S[7] = S[5], S[8] = (double)GAL_NUM + nn - 2 * tr_QN3 + tr_QN3_2;
        matinv(S, 3);
        matmul("NN", 3, 1, 3, 1.0, S, W, 0.0, delta);
        del1 = delta[0], del2 = delta[1], del3 = delta[2];

        /*以下为方差分量简版计算方法，一般不采用*/
        //double del1 = W[0] / ((double)GPS_NUM - tr_QN1);
        //double del2 = W[1] / ((double)GLO_NUM - tr_QN2);
        //double del3 = W[2] / ((double)GAL_NUM + nn - tr_QN3);

if (del1 <= 0 || del2 <= 0 || del3 <= 0) {
            free(v1), free(v2), free(v3), free(B1), free(B2), free(B3);
            return 0;
        }
        if ((fabs(del1 - del2) < 0.001) && (fabs(del1 - del3) < 0.001) && (fabs(del2 - del3) < 0.001))break;
        else {
            for (i = 0; i < GPS_NUM; i++) {
                v[i] /= sqrt(del1);
                for (j = 0; j < NX; j++)  H[j + i * NX] /= sqrt(del1);
            }
            for (i = 0; i < GLO_NUM; i++) {
                v[i + GPS_NUM] /= sqrt(del2);
                for (j = 0; j < NX; j++)  H[j + (i + GPS_NUM) * NX] /= sqrt(del2);
            }
            for (i = 0; i < GAL_NUM + nn; i++) {
                v[i + GLO_NUM + GPS_NUM] /= sqrt(del3);
                for (j = 0; j < NX; j++)  H[j + (i + GLO_NUM + GPS_NUM) * NX] /= sqrt(del3);
            }
            lsq(H, v, NX, nv, dx, Q);
        }
    }
...
static int filter_(const double *x, const double *P, const double *H,
                   ...
                   double *xp, double *Pp,rtk_t*rtk)
{
    double *F=mat(n,m),*Q=mat(m,m),*K=mat(n,m),*I=eye(n);
    double* KR = mat(n, m), * Pp_ = mat(n, n), * I_ = eye(n);
    double* r = zeros(m, 1), * MDBs = mat(m, 1), * MDBE = mat(6, 1);
    double* tmp1 = mat(1, m), * tmp2 = mat(1, m), * M = mat(1, 1);
    double* tmp3 = mat(n, 1), * ME = mat(n, 1), * MEs = mat(n, m);
    int info;
...
/* 新,修改P计算方法以提高数值稳定性 */
		matcpy(Pp_,Pp,n,n);
		matmul("NT",n,n,n,1.0,Pp_,I,0.0,Pp);
		matmul("NN",n,m,m,1.0,K,R,0.0,KR);
		matmul("NT",n,n,m,1.0,KR,K,0.0,Pp_);
		matmul("NN",n,n,n,1.0,Pp_,I_,1.0,Pp);
}

    /*For the calculation of Internal and external reliability namely, MDBs and ME(Using innovation vector) 20250101*/
    /*When the confidence level ahpha is 0.05 and the test power gamma is 0.8, λ(α，γ)=2.8*2.8*/
    if (!(info = matinv(R, m))) {
        for (int i = 0;i < m;i++) {
            r[i] = 1;/*某个双差观测值对应r的位置*/
            matmul("TN", 1, m, m, 1.0, r, R, 0.0, tmp1);
            matmul("NN", 1, 1, m, 1.0, tmp1, r, 0.0, tmp2);
            M[0] = tmp2[0] > 0 ? ((1.96 + 0.8416) / (sqrt(tmp2[0]))) : 0.0;
            MDBs[i] = fabs(M[0]);
            matmul("NN", n, 1, m, 1.0, K, r, 0.0, tmp3);
            matmul("NN", n, 1, 1, 1.0, tmp3, M, 0.0, ME);/*external reliability ME*/
            for (int j = 0;j < n;j++) {
                /*cal ceof for position or ambiguity--xyz GPS BDS*/
                double coef = j < 3 ? 1 : (j < 3 + rtk->numnv[0] + 1 ? CLIGHT / FREQL1 : CLIGHT / FREQ1_CMP);
                MEs[i * n + j] = coef * fabs(ME[j]);
            }
            r[i] = 0;/*计算完之后将该观测值对应的r置为0循环下一个观测值*/
        }
    }
if (statistic_func(MDBs, MEs, m, n, rtk->numnv, 0, MDBE)) {/* all phase*/
        for (int i = 0;i < 6;i++) {
            rtk->sol.MDBE[i] = MDBE[i];
        }
    }
    if (statistic_func(MDBs, MEs, m, n, rtk->numnv, 1, MDBE)) {/*all code*/
        for (int i = 0;i < 6;i++) {
            rtk->sol.MDBE[i + 6] = MDBE[i];
        }
    }
    /*For the calculation of Internal and external reliability namely, MDBs and ME20250101*/
	free(F); free(Q); free(K); free(I);
	free(KR);free(Pp_);free(I_);
    free(r);free(MDBs);free(MDBE);
    free(tmp1);free(tmp2);free(M);
    free(tmp3);free(ME);free(MEs); 

	return info;
}
/* statistic reliability indicators---------------------------------------------------------------
*
* args   : double *MDBs      I   The MDBs of dd obs (m x 1)
*          double *MEs       I   The MEs of all obs-param (n x m)
*          int    m n        I   number of obs and param
*          int    nvnum      I   The num of code or phase dd residual
*          int    flag       I   flag of code or phase
*          double *MDBE      I   statistic results (6*1)
* return : 1
*-----------------------------------------------------------------------------*/
static int statistic_func(double* MDBs, double* MEs, int m,int n, const int nvnum[4], int flag, double* MDBE) {
    /*Index code or phase observations 0-phase,1-code*/
    int idx1 = nvnum[0], idx2 = nvnum[0] + nvnum[1], idx3 = nvnum[0] + nvnum[1] + nvnum[2], numamb = 0;
    int num1 = flag ? idx1 : 0, num2 = flag ? idx2 : idx1;
    int num3 = flag ? idx3 : idx2, num4 = flag ? m : idx3;
    int allnum = flag ? nvnum[1] + nvnum[3] : nvnum[0] + nvnum[2];
    double meanMDB = 0.0, meanME_xzy = 0.0, meanME_amb = 0.0;
    double maxMDB = -INFINITY, maxME_xyz = -INFINITY, maxME_amb = -INFINITY;
    double mdb, rmsME_xyz, tmpME_amb;
    for (int i = 0;i < m;i++) {
        if (i < num2 && i >= num1) {/*GPS code or phase*/
            mdb = MDBs[i];
            if (mdb > maxMDB) maxMDB = mdb;
            meanMDB += mdb;
            /*ME for xyz*/
            rmsME_xyz = sqrt(MEs[i * n] * MEs[i * n] + MEs[i * n + 1] * MEs[i * n + 1] + MEs[i * n + 2] * MEs[i * n + 2]);
            if (rmsME_xyz > maxME_xyz) maxME_xyz = rmsME_xyz;
            meanME_xzy += rmsME_xyz;
            for (int j = 3;j < n - (nvnum[2] + 1);j++) {/*cal ME for GPS amb*/
                tmpME_amb = MEs[i * n + j];
                if (tmpME_amb > maxME_amb) maxME_amb = tmpME_amb;
                meanME_amb += tmpME_amb;
                numamb++;
            }
        }
        else if (i < num4 && i >= num3) {/*BDS code or phase*/
            mdb = MDBs[i];
            if (mdb > maxMDB) maxMDB = mdb;
            meanMDB += mdb;
            /*ME for xyz*/
            ...
            if (rmsME_xyz > maxME_xyz) maxME_xyz = rmsME_xyz;
            meanME_xzy += rmsME_xyz;
            for (int j = n - (nvnum[2] + 1);j < n;j++) {/*cal ME for BDS amb*/
                tmpME_amb = MEs[i * n + j];
                if (tmpME_amb > maxME_amb) maxME_amb = tmpME_amb;
                meanME_amb += tmpME_amb;
                numamb++;
    ...
    MDBE[0] = meanMDB/(allnum);MDBE[1] = maxMDB;MDBE[2] = meanME_xzy / (allnum);
    MDBE[3] = maxME_xyz;MDBE[4] = meanME_amb / (numamb);MDBE[5] = maxME_amb;
    return 1;
}
...
extern int filter(double *x, double *P, const double *H, const double *v,
                  const double *R, int n, int m,rtk_t *rtk)
{
    ...
    info=filter_(x_,P_,H_,v,R,k,m,xp_,Pp_,rtk);
