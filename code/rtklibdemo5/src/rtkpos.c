...
const int IGSO[] = { 6,7,8,9,10,13,16,38,39,40 };
/* constants/macros ----------------------------------------------------------*/
...
// 用于输出model权值的文件
extern int openmodel(const char* file)
{
	gtime_t time = utc2gpst(timeget());
	char path[1024];
...
static void outsolstat(rtk_t* rtk, const nav_t* nav)
{
	...
	if (rtk->nfix == 0) return;/*test by qq*/
	...
	for (i = 0; i < MAXSAT; i++) {
		...
		for (j = 0; j < nfreq; j++) {
			...
			/*fprintf(fp_stat, "$SAT,%d,%.3f,%s,%d,%.1f,%.1f,%.4f,%.4f,%d,%.0f,%d,%d,%d,%d,%d,%d,%.2f,%.6f,%.5f,%.5f\n",
				...
				/*test for PLD by qq*/
			fprintf(fp_stat, "$SAT,%d,%.3f,%s,%d,%.1f,%.1f,%.4f,%.4f,%d,%.0f,%.0f,%.1f,%.1f,%d,%d,%d,%d,%d,%d,%.2f,%.6f,%.5f,%.5f,%.4f\n",
				week, tow, id, j + 1, ssat->azel[0] * R2D, ssat->azel[1] * R2D,
				ssat->resp[j], ssat->resc[j], ssat->vsat[j], ssat->snr_rover[j] * 0.25,
				ssat->pld_rover[j] * 1.0, ssat->rv_rover[j], ssat->cv_rover[j],
				ssat->fix[j], ssat->slip[j] & 3, ssat->lock[j], ssat->outc[j],
				ssat->slipc[j], ssat->rejc[j], dgps ? 0 : rtk->x[k],
				dgps ? 0 : rtk->P[k + k * rtk->nx], nav->lam[i][j], ssat->icbias[j], ssat->deltcmc);
		}
	}
}
/* save error message --------------------------------------------------------*/
...
/*判断数组内是否有某数值*/
static int isValueInset(int value, const int* arry, int size) {
...
/*Ele-C/N0 Template function*/
static double snr_temp(int sys, double Ele, int prn)
{
	if (sys == SYS_GPS) {
		return 37.86 + 0.4317 * Ele - 0.005024 * pow(Ele, 2) + 2.062e-5 * pow(Ele, 3);
	}
	else if (sys == SYS_CMP) {
		if (isValueInset(prn, IGSO, 10)) {
			return 39.14 + 0.1665 * Ele + 0.0006956 * pow(Ele, 2) - 1.374e-5 * pow(Ele, 3);
		}
		else { /*MEO*/
			return 39.45 + 0.4358 * Ele - 0.005515 * pow(Ele, 2) + 2.462e-5 * pow(Ele, 3);
		}
	}
	return 0;
}
/*Ele-C/N0 sigma Template fuction*/
static double snr_std_temp(int sys, double Ele, int prn)
{
	if (sys == SYS_GPS) {
		return 4.313 - 0.1273 * Ele + 0.001595 * pow(Ele, 2) - 4.847e-6 * pow(Ele, 3);
	}
	else if (sys == SYS_CMP) {
		if (isValueInset(prn, IGSO, 10)) {
			return 2.51 - 0.06479 * Ele + 0.001224 * pow(Ele, 2) - 7.565e-6 * pow(Ele, 3);
		}
		else {
			return 4.264 - 0.1936 * Ele + 0.003598 * pow(Ele, 2) - 2.105e-5 * pow(Ele, 3);
		}
	}
	return 0;
}
/*C/N0-PLD Template function*/
static double pld_temp(int sys, double snr, int prn)
{
	if (sys == SYS_GPS) {
		return 4.27635751E+03 - 3.38181340E+02 * snr + 1.00466429E+01 * pow(snr, 2) - 1.32654859E-01 * pow(snr, 3) + 6.56662716E-04 * pow(snr, 4);
	}
	else if (sys == SYS_CMP) {
		if (isValueInset(prn, IGSO, 10)) {
			return 4.63039160E+03 - 3.71820029E+02 * snr + 1.12210562E+01 * pow(snr, 2) - 1.50587145E-01 * pow(snr, 3) + 7.58002370E-04 * pow(snr, 4);
		}
		else {
			return 4.16074457E+03 - 3.30819016E+02 * snr + 9.89204578E+00 * pow(snr, 2) - 1.31606980E-01 * pow(snr, 3) + 6.57136680E-04 * pow(snr, 4);
		}
	}
	return 0;
}

/*C/N0-PLD sigma Template function*/
static double pld_std_temp(int sys, double snr, int prn)
{
	if (sys == SYS_GPS) {
		return 3.42677171E+02 - 1.86704532E+01 * snr + 3.37901186E-01 * pow(snr, 2) - 2.03023968E-03 * pow(snr, 3);
	}
	else if (sys == SYS_CMP) {
		if (isValueInset(prn, IGSO, 10)) {
			return 3.04806142E+02 - 1.66969599E+01 * snr + 3.03616994E-01 * pow(snr, 2) - 1.83050211E-03 * pow(snr, 3);
		}
		else {
			return 3.18744547E+02 - 1.78108649E+01 * snr + 3.32428902E-01 * pow(snr, 2) - 2.07211122E-03 * pow(snr, 3);
		}
	}
	return 0;
}
/*CN0 Phase variance template function*/
static double CV_temp(int sys, double snr, int prn) {
	if (sys == SYS_GPS) {
		return 2.878595E+04 - 1.288069E+03 * snr + 2.028803E+01 * pow(snr, 2) - 1.067007E-01 * pow(snr, 3);
	}
	else if (sys == SYS_CMP) {
		if (isValueInset(prn, IGSO, 10)) {
			return 3.110906E+04 - 1.439525E+03 * snr + 2.349143E+01 * pow(snr, 2) - 1.289873E-01 * pow(snr, 3);
		}
		else {
			return 3.361127E+04 - 1.605184E+03 * snr + 2.712352E+01 * pow(snr, 2) - 1.553730E-01 * pow(snr, 3);
		}
	}
	return 0;
}

static double CV_std_temp(int sys, double snr, int prn) {
	if (sys == SYS_GPS) {
		return 5.498673E+03 - 2.672540E+02 * snr + 4.278887E+00 * pow(snr, 2) - 2.241719E-02 * pow(snr, 3);
	}
	else if (sys == SYS_CMP) {
		if (isValueInset(prn, IGSO, 10)) {
			return 1.080565E+04 - 6.012852E+02 * snr + 1.118574E+01 * pow(snr, 2) - 6.942111E-02 * pow(snr, 3);
		}
		else {
			return 4.366496E+03 - 2.108469E+02 * snr + 3.315637E+00 * pow(snr, 2) - 1.667602E-02 * pow(snr, 3);
		}
	}
	return 0;
}

/*Phase variance-Code variance template function*/
static double RV_temp(int sys, double C_V, int prn) {
	if (sys == SYS_GPS) {
		return -3.676222E+05 * pow(C_V, -3.229797E-01) + 3.966197E+04;
	}
	else if (sys == SYS_CMP) {
		if (isValueInset(prn, IGSO, 10)) {
			return -1.519670E+06 * pow(C_V, -5.899051E-01) + 2.543366E+04;
		}
		else {
			return -2.250166E+05 * pow(C_V, -9.237937E-02) + 1.197139E+05;
		}
	}
	return 0;
}
static double RV_std_temp(int sys, double C_V, int prn) {
	if (sys == SYS_GPS) {
		return 1.590715E+02 - 2.969497E-01 * C_V + 1.823968E-04 * pow(C_V, 2);
	}
	else if (sys == SYS_CMP) {
		if (isValueInset(prn, IGSO, 10)) {
			return 6.690248E+02 - 8.328818E-01 * C_V + 3.176798E-04 * pow(C_V, 2);
		}
		else {
			return 7.227487E+01 - 2.205269E-01 * C_V + 1.619731E-04 * pow(C_V, 2);
		}
	}
	return 0;
}
...
/*R_V : code range variance
  C_V: carrier phase variance*/
static double varerr(int sat, int sys, double el, double snr_rover, double snr_base, double pld_rover, double R_V, double C_V,
	double bl, double dt, int f, const prcopt_t* opt, const obsd_t* obs)
{
	...
	double fact = 1.0;
	double sinel = sin(el);
	int i, nf = NF(opt), frq, prn, code;
	int cp_ind; // PH-区分伪距和载波定权的大小
	double final_weight;
	...
	double temp_snr, temp_pld, thres_snr, thres_pld, temp_CV, std_CV, temp_RV, std_RV;
	temp_snr = snr_temp(sys, el * R2D, prn);
	temp_pld = pld_temp(sys, temp_snr, prn);
	thres_snr = snr_std_temp(sys, el * R2D, prn);
	thres_pld = pld_std_temp(sys, temp_snr, prn);
	/*phase and code variance*/
	temp_CV = CV_temp(sys, temp_snr, prn);
	std_CV = CV_std_temp(sys, temp_snr, prn);
	temp_RV = RV_temp(sys, temp_CV, prn);
	std_RV = RV_std_temp(sys, temp_CV, prn);

	double eleWeight, snrWeight, PLDWeight, allWeight;
	...
	/* 单一指标*/
	eleWeight = 2 * (fact * (SQR(a) + SQR(b / sinel) + SQR(c)) + SQR(d));

	snrWeight = fact * (SQR(a) * (pow(10, 0.1 * MAX(snr_max - snr_rover, 0)) +
		pow(10, 0.1 * MAX(snr_max - snr_base, 0))) + SQR(c)) + SQR(d);

	/*PLD*/
	if (sys == SYS_GPS) {/*phase*/
		PLDWeight = 2 * fact * pow((-1.18094351E-03 + 1.18737460E-03 * pld_rover - 2.20456704E-05 * pow(pld_rover, 2) +
			2.15611753E-07 * pow(pld_rover, 3) - 7.42187119E-10 * pow(pld_rover, 4)), 2) + SQR(d);
	}
	else if (sys == SYS_CMP) {
		PLDWeight = 2 * fact * pow((-2.38054087E-03 + 1.03295252E-03 * pld_rover - 1.13286920E-05 * pow(pld_rover, 2) +
			3.03708286E-08 * pow(pld_rover, 3) + 5.93967980E-11 * pow(pld_rover, 4)), 2) + SQR(d);
	}
	if (code) {/*code*/
		if (sys == SYS_GPS) {
			PLDWeight *= pow(((-1.25768773E+01 * pld_rover + 4.23648975E+03) / (pld_rover + 2.25978165E+01)), 2);
		}
		else if (sys == SYS_CMP) {
			PLDWeight *= pow(((5.02419156E+01 * pow(pld_rover, 2) - 1.33948552E+03 * pld_rover + 2.63684592E+04) / (pow(pld_rover, 2)
				- 2.32139789E+01 * pld_rover + 2.23513435E+02)), 2);
		}
	}

	//if (f < nf) {
	//	if (sys == SYS_GPS) {
	//		PLDWeight = fact * pow(0.001224 * pow(pld_rover, 0.5031), 2) + SQR(d);
	//	}
	//	else if (sys == SYS_CMP) {
	//		PLDWeight = fact * pow((-0.04731 * pow(pld_rover, -0.1916) + 0.03482), 2) + SQR(d);
	//	}
	//}
	//else if (f == nf) {
	//	if (sys == SYS_GPS) {
	//		PLDWeight = fact * (1.046 * exp(0.01142 * pld_rover) - 1.472 * exp(-0.1349 * pld_rover)) + SQR(d);
	//	}
	//	else if (sys == SYS_CMP) {
	//		PLDWeight = fact * (2.138 * exp(0.003915 * pld_rover) - 2.716 * exp(-0.07327 * pld_rover)) + SQR(d);
	//	}
	//}
	//PLDWeight *= 2;/*SD-DD*/

	/*双指标*/
	double ele_snr, ele_pld, snr_pld, ele_snrWeight, ele_pldWeight, snr_pldWeight;
	if (sys == SYS_GPS) {
		ele_snr = 0.1252 * snrWeight + 0.8748 * eleWeight;
		ele_pld = 0.1331 * eleWeight + 0.8669 * PLDWeight;
		snr_pld = 0.8628 * snrWeight + 0.1372 * PLDWeight;
	}
	else if (sys == SYS_CMP) {
		ele_snr = 0.2791 * snrWeight + 0.7209 * eleWeight;
		ele_pld = 0.1158 * eleWeight + 0.8842 * PLDWeight;
		snr_pld = 0.8845 * snrWeight + 0.1155 * PLDWeight;
	}
	double aver1 = (eleWeight + snrWeight) / 2, thre1 = SQRT((SQR(eleWeight - aver1) + SQR(snrWeight - aver1)) / 1);
	double aver2 = (eleWeight + PLDWeight) / 2, thre2 = SQRT((SQR(eleWeight - aver2) + SQR(PLDWeight - aver2)) / 1);
	double aver3 = (snrWeight + PLDWeight) / 2, thre3 = SQRT((SQR(snrWeight - aver3) + SQR(PLDWeight - aver3)) / 1);
	ele_snrWeight = fabs(eleWeight - aver1) <= thre1 ? (fabs(snrWeight - aver1) <= thre1 ? ele_snr : eleWeight) : ((fabs(snrWeight - aver1) > thre1) ? 10000 * aver1 : snrWeight);
	ele_pldWeight = fabs(eleWeight - aver2) <= thre2 ? (fabs(PLDWeight - aver2) <= thre2 ? ele_pld : eleWeight) : ((fabs(PLDWeight - aver2) > thre2) ? 10000 * aver2 : PLDWeight);
	snr_pldWeight = fabs(snrWeight - aver3) <= thre3 ? (fabs(PLDWeight - aver3) <= thre3 ? snr_pld : snrWeight) : ((fabs(PLDWeight - aver3) > thre3) ? 10000 * aver3 : PLDWeight);

	/*综合模型*/
	double averange = (eleWeight + snrWeight + PLDWeight) / 3;
	double thre0 = SQRT((SQR(eleWeight - averange) + SQR(snrWeight - averange) + SQR(PLDWeight - averange)) / 2);
	int ele_use = fabs(eleWeight - averange) <= thre0;
	int snr_use = fabs(snrWeight - averange) <= thre0;
	int pld_use = fabs(PLDWeight - averange) <= thre0;
	double deltsnr = fabs(temp_snr - snr_rover);
	double deltpld = fabs(temp_pld - pld_rover);
	int usesnr = deltsnr <= 2 * thres_snr;
	int usepld = deltpld <= 2 * thres_pld;
	if (usesnr && usepld) {
		if (ele_use && snr_use && pld_use) {
			allWeight = averange;
		}
		else if (!ele_use && snr_use && pld_use) allWeight = aver3;
		else if (ele_use && !snr_use && pld_use) allWeight = aver2;
		else if (ele_use && snr_use && !pld_use) allWeight = aver1;
		else {
			allWeight = pld_use ? PLDWeight : (snr_use ? snrWeight : (ele_use ? eleWeight : averange * 1e5));
		}
		/*else {
			allWeight = pld_use ? (snr_use ? aver3 : PLDWeight) : (snr_use ? snrWeight : (ele_use ? eleWeight : averange * 1e5));
		}*/
	}
	else if (usesnr && !usepld) {
		/*if (fabs(eleWeight - aver1) <= thre1 && fabs(snrWeight - aver1) > thre1) {
			allWeight = eleWeight;
		}
		else {
			allWeight = snrWeight;
		}*/
		allWeight = 0.4 * aver1 + 0.6 * PLDWeight;
	}
	else if (!usesnr && usepld) {
		/*if (fabs(eleWeight - aver2) <= thre2 && fabs(PLDWeight - aver2) > thre2) {
			allWeight = eleWeight;
		}
		else {
			allWeight = PLDWeight;
		}*/
		allWeight = 0.6 * snrWeight + 0.4 * PLDWeight;
	}
	else if (!usesnr && !usepld) {
		if (deltsnr > 3 * thres_snr && deltpld > 3 * thres_pld && fabs(PLDWeight - aver3) > thre3 && fabs(snrWeight - aver3) > thre3) {
			allWeight = averange * 1e5;
		}
		else {
			allWeight = aver3;
		}
		...
		/*if (deltsnr > 3 * thres_snr && deltpld > 3 * thres_pld) {
			allWeight = fabs(PLDWeight - aver3) <= thre3 ? (fabs(snrWeight - aver3) <= thre3 ? aver3 : snrWeight) : (fabs(snrWeight - aver3) <= thre3 ? PLDWeight : averange * 1e5);
		}
		else if (deltsnr > 3 * thres_snr && deltpld <= 3 * thres_pld) {
			allWeight = 0.8 * snrWeight + 0.2 * PLDWeight;
		}
		else if (deltsnr <= 3 * thres_snr && deltpld > 3 * thres_pld) {
			allWeight = 0.2 * snrWeight + 0.8 * PLDWeight;
		}
		else {
			allWeight = aver3;
		}*/
		...
		/*if (deltsnr > 3 * thres_snr && deltpld > 3 * thres_pld) {
			allWeight = pld_use ? (snr_use ? aver3 : PLDWeight) : (snr_use ? snrWeight : averange * 1e5);
		}
		else if (deltsnr > 3 * thres_snr && deltpld <= 3 * thres_pld) {
			allWeight = PLDWeight;
		}
	...
	if (fp_model)
	{
		//gtime_t time = gpst2utc(obs->time);
		int time = obs->time.time;
		double sec = obs->time.sec;
		//printf("%d,%.2f,%d,%d,%.5f,%.3f,%.2f,%.6f,%.6f,%.6f\n", time, sec, sat, sys, el, snr_rover, pld_rover, eleWeight, snrWeight, PLDWeight);
		fprintf(fp_model, "%d,%.2f,%d,%d,%d,%.5f,%.3f,%.2f,%.10f,%.10f,%.10f,%.10f\n", time, sec, sys, sat, f, el, snr_rover, pld_rover, eleWeight, snrWeight, PLDWeight, allWeight);
	...
	case WEIGHTOPT_ELEVATION: final_weight = eleWeight; break;

	case WEIGHTOPT_SNR: final_weight = snrWeight; break;

	case WEIGHTOPT_PLD: final_weight = PLDWeight; break;

	case WEIGHTOPT_ALL: final_weight = allWeight; break;

	case WEIGHTOPT_ALL_CPV:final_weight = allWeight; break;

	case WEIGHTOPT_ELE_SNR:
		//return eleWeight * snrWeight;
		final_weight = ele_snrWeight; break;

	case WEIGHTOPT_ELE_PLD:
		final_weight = ele_pldWeight; break;

	case WEIGHTOPT_SNR_PLD:
		final_weight = snr_pldWeight; break;

	default: final_weight = 0; break;
	}

	/*use the defined macros to control code and phase variance*/

	/*code or phase accuracy*/
	double obs_accu;
	if (code) {/*code*/
		if (sys == SYS_GPS) {
			obs_accu = 2 * fact * pow((2.479547 - 8.632286E-4 * R_V + 1.126853E-7 * pow(R_V, 2) - 4.188011E-12 * pow(R_V, 3)), 2);
		}
		else if (sys == SYS_CMP) {
			obs_accu = 2 * fact * pow((4.723421 - 1.689601E-3 * R_V + 2.037103E-7 * pow(R_V, 2) - 6.734449E-12 * pow(R_V, 3)), 2);
		}
	}
	else {
		if (sys == SYS_GPS) {
			obs_accu = 2 * fact * pow((5.676337E-3 - 1.279478E-5 * C_V + 8.888894E-9 * pow(C_V, 2) - 1.405074E-12 * pow(C_V, 3)), 2);
		}
		else if (sys == SYS_CMP) {
			obs_accu = 2 * fact * pow((-3.336582E-2 + 4.482758E-5 * C_V - 1.876500E-8 * pow(C_V, 2) + 2.972035E-12 * pow(C_V, 3)), 2);
		}
	}
	/* Use code and phase variance to refine the compre stochastic model*/
	if (opt->weightmode == WEIGHTOPT_ALL_CPV) {
		double aver_accu = (obs_accu + final_weight) / 2;
		double thre_accu = SQRT((SQR(obs_accu - aver_accu) + SQR(final_weight - aver_accu)) / 1);
		/*phase or code variance measurement and template value*/
		double deltCRV = code ? fabs(temp_RV - R_V) : fabs(temp_CV - C_V);
		double stdCRV = code ? std_RV : std_CV;
		if (deltCRV > 2 * stdCRV && deltCRV <= 3 * stdCRV) {/* 2std<dlet<=3std */
			final_weight = aver_accu;
		}
		else if (deltCRV > 3 * stdCRV && deltCRV <= 4.5 * stdCRV) {
			if (final_weight > 3 * thre_accu && obs_accu <= 3 * thre_accu) {
				final_weight = 0.7 * obs_accu + 0.3 * final_weight;
			}
			else if (obs_accu > 3 * thre_accu && final_weight <= 3 * thre_accu) {
				final_weight = 0.3 * obs_accu + 0.7 * final_weight;
			}
			else {
				final_weight = aver_accu;
			}
	...
	if (opt->weightmode == WEIGHTOPT_CV_PV) final_weight = obs_accu;
	return final_weight;
}
...
return;
	...
	//////////////////////////////////////////////////////////////////////////AFTER THIS ,don't consider fault mode
	...
	const nav_t* nav)
{
	...
	if (NFREQ <= 1 || (g1 = gfobs_L1L2(obs, i, j, nav->lam[sat - 1])) == 0.0) return;
...
/*use code minus carrier phase only for the PLD_data*/
static void detslp_CMC(rtk_t* rtk, const obsd_t* obs, int i, const nav_t* nav) {
	int sat = obs[i].sat;
	double cmc0, cmc1, lam;
	/*obtain cmc obs from code and phase*/
	lam = nav->lam[sat - 1][0];
	if (obs[i].P[0] == 0.0 || obs[i].L[0] == 0.0) return;

	cmc1 = obs[i].P[0] - lam * obs[i].L[0];
	cmc0 = rtk->ssat[sat - 1].cmc; /* pre cmc*/
	rtk->ssat[sat - 1].cmc = cmc1;/*save current cmc*/
	double cmcthresslip = 0.5;
	if (cmc0 != 0.0) rtk->ssat[sat - 1].deltcmc = fabs(cmc1 - cmc0);
	else  rtk->ssat[sat - 1].deltcmc = 0;

	if (cmc0 != 0.0 && fabs(cmc1 - cmc0) > cmcthresslip) {

		rtk->ssat[sat - 1].slip[0] |= 1;

		errmsg(rtk, "slip detected CMC_jump (sat=%2d CMC=%.3f %.3f)\n", sat, cmc0, cmc1);
	}
}
	...
	const int* iu, const int* ir, int ns, const nav_t* nav)
{
	...
	for (i = 0; i < ns; i++) {
		...
		//detslp_ll(rtk,obs,iu[i],1);  //取消LLI探测
		//detslp_ll(rtk,obs,ir[i],2);  //取消LLI探测

		/* detect cycle slip by geometry-free phase jump */
		detslp_gf_L1L2(rtk, obs, iu[i], ir[i], nav);
		detslp_gf_L1L5(rtk, obs, iu[i], ir[i], nav);

		/* detect clock jump by code minus phase jump */
		detslp_CMC(rtk, obs, iu[i], nav);
	...
	for (f = 0; f < nf; f++) {
		...
		for (i = 1; i <= MAXSAT; i++) {
			...
			else if (reset && rtk->x[IB(i, f, &rtk->opt)] != 0.0) {/*连续多个历元卫星不可用*/
				initx(rtk, 0.0, 0.0, IB(i, f, &rtk->opt));
...
if (rtk->opt.modear != ARMODE_INST && reset) {
				rtk->ssat[i - 1].lock[f] = -rtk->opt.minlock;/*卫星一直没有观测值*/
			}
		}
...
slip = rtk->ssat[sat[i] - 1].slip[f];
			...
			if (rtk->opt.modear == ARMODE_INST || (!(slip & 1) && rejc < 4)) continue;/*cycle slips or rejct counters 2 epochs*/
			rtk->x[j] = 0.0;/*init ambiguity*/

if (slip & 1) {
				rtk->ssat[sat[i] - 1].lock[f] = -5;/*slip cycle*/
			}
			else {
				rtk->ssat[sat[i] - 1].rejc[f] = 0;/*rejct*/
			}
...
//这个地方得细改
		...
		for (i = j = 0, offset = 0.0; i < ns; i++) {
			...
			//这个地方考虑所有卫星的共同偏差，就不考虑每个子集单独剔除卫星再求和了
		}
	...
	const prcopt_t* opt, double* y)
{
	...
	if (opt->ionoopt == IONOOPT_IFLC) { /* iono-free linear combination */
		...
		for (i = 0; i < nf; i++) {
			...
			/*test PLD*/
			if (opt->weightmode == 2 || opt->weightmode == 3 || opt->weightmode == 5 || opt->weightmode == 6) {
				if (obs->PLD[0] && obs->PLD[0] > opt->chkMaxPLD)//如果PLD大于多少就剔除
				{
					continue;
				}
			}
	...
	double* H, double* R, int* vflg)
{
	...
	int i, j, ii, jj, k, m, f, frq, code, nv = 0, nb[NFREQ * 4 * 2 + 2] = { 0 }, b = 0, sysi, sysj, nf = NF(opt), aaa, index;

	trace(3, "ddres   : dt=%.1f nx=%d ns=%d\n", dt, rtk->nx, ns);

	for (i = 0;i < 4;i++) rtk->numnv[i] = 0;/*init num*/
	...
	for (m = 0; m < 5; m++) {
		...
		for (f = opt->mode > PMODE_DGPS ? 0 : nf; f < nf * 2; f++) {
			...
			for (j = 0; j < ns; j++) {
				...
				threshadj = (rtk->P[ii + rtk->nx * ii] > SQR(rtk->opt.std[0] / 2)) ||
					(rtk->P[jj + rtk->nx * jj] > SQR(rtk->opt.std[0] / 2)) ? 5 : 1;
				if (opt->maxinno > 0.0 && fabs(v[nv]) > opt->maxinno * threshadj) {
					if (f < nf) {/*only for the phase observation*/
				...
				/*store the num of code or phase of each sys 20250105*/
				if (code) index = sysj == SYS_GPS ? 1 : 3;/*默认GPS+BDS双系统*/
				else      index = sysj == SYS_GPS ? 0 : 2;
				rtk->numnv[index]++;
				/*store the num of code or phase of each sys 20250105*/
				...
				Ri[nv] = varerr(sat[i], sysi, azel[1 + iu[i] * 2],
					...
					rtk->ssat[sat[i] - 1].pld_rover[frq],
					rtk->ssat[sat[i] - 1].rv_rover[frq],
					rtk->ssat[sat[i] - 1].cv_rover[frq],
				...
				Rj[nv] = varerr(sat[j], sysj, azel[1 + iu[j] * 2],
					...
					rtk->ssat[sat[j] - 1].pld_rover[frq],
					rtk->ssat[sat[j] - 1].rv_rover[frq],
					rtk->ssat[sat[j] - 1].cv_rover[frq],
...
/* 新，抗差卡尔曼滤波,无需二次滤波[面向大众导航的低成本接收机高精度定位算法]-------*/
static int robust_KF2(rtk_t* rtk, const double* x, const double* P, const double* v,
	const double* H, double* R, int nx, int nv)
{
	double* vk, * P_, * R_, * Q_vk, * Q_vkk, * HP, * RQ, * H_, * Gamma;
	double vi, k0 = 2.0/*2.0-3.0*/, k1 = 8.5/*4.5-8.5*/, gamma = 1.0, gamma_i = 1.0, gamma_j = 1.0, c = 3.5;
	int i, j, n = nx, info = 1, * ix;
	...
	vk = mat(nv, 1);  P_ = mat(n, n);      HP = mat(nv, n);  Q_vk = mat(nv, nv);
	R_ = mat(nv, nv); Q_vkk = mat(nv, nv); RQ = mat(nv, nv); Gamma = mat(nv, 1);
	H_ = mat(n, nv);

	for (i = 0; i < n; i++) {
		for (j = 0; j < n; j++) P_[i + j * n] = P[ix[i] + ix[j] * nx];
		for (j = 0; j < nv; j++) H_[i + j * n] = H[ix[i] + j * nx];
	}

	/*matcpy(P_,P,n,n); */
	matcpy(Q_vk, R, nv, nv);   matcpy(R_, R, nv, nv);
	matmul("TN", nv, n, n, 1.0, H_, P_, 0.0, HP);    /* HP=HT*P */
	matmul("NN", nv, nv, n, 1.0, HP, H_, 1.0, Q_vk); /* Q=R+HT*P*H */

	if (!(info = matinv(Q_vk, nv))) {
		matmul("NN", nv, nv, nv, 1.0, R_, Q_vk, 0.0, RQ);  /* RQ=R*Q^-1 */
		matmul("NN", nv, 1, nv, 1.0, RQ, v, 0.0, vk);      /* vk=R*Q^-1*v */
		matmul("NT", nv, nv, nv, 1.0, RQ, R_, 0.0, Q_vkk); /* Q_=R*Q^-1*RT */
	}

	for (i = 0; i < nv; i++) {
		//20240321 by qq
		vi = fabs(vk[i]) / SQRT(Q_vkk[i * nv + i]);
		/*Huber*/
		if (vi < c) {
			gamma = 1.0;
		}
		...
		{
			gamma = k0 / vi * pow((k1 - vi) / (k1 - k0), 2);
			printf("down----\r\n");
		}*/
		Gamma[i] = gamma;
		//R[i*nv+i]=R[i*nv+i]/gamma;
	}
	/* 调整方差及协方差[BDS/GPS组合动态相对定位及其质量控制方法研究] */
	for (i = 0; i < nv; i++) {
		gamma_i = Gamma[i];
		for (j = 0; j < nv; j++) {
			gamma_j = Gamma[j];
			gamma = SQRT(gamma_i * gamma_j);
			R[i * nv + j] = R[i * nv + j] / gamma;
		}
	}

	free(vk); free(P_); free(Q_vk); free(Gamma);
	free(RQ); free(Q_vkk); free(HP); free(R_);
	free(ix); free(H_);
	...
	for (m = 0; m < 5; m++) { /* m=0:gps/sbs,1:glo,2:gal,3:bds,4:qzs */
		...
		for (f = 0, k = na; f < nf; f++, k += MAXSAT) {
			...
			for (j = k; j < k + MAXSAT; j++) {
				...
				if (1 && satsys(j - k + 1, NULL) == SYS_SBS) continue;
				if (rtk->ssat[j - k].lock[f] >= 0 && !(rtk->ssat[j - k].slip[f] & 2) &&
					rtk->ssat[i - k].vsat[f] &&
					rtk->ssat[j - k].azel[1] >= rtk->opt.elmaskar && !nofix) {
...
{
	int i, n[5], m, f, index[MAXSAT], nv = 0, nf = NF(&rtk->opt), fixsatnum = rtk->opt.sizes[0] + rtk->opt.sizes[1];
	trace(3, "restamb :\n");
	int flag = 1, info, fixsatnumG = rtk->opt.sizes[0], fixsatnumC = rtk->opt.sizes[1];
	...
	for (m = 0; m < 5; m++) for (f = 0; f < nf; f++) {
		...
		for (n[m] = i = 0; i < MAXSAT; i++) {
			if (!test_sys(rtk->ssat[i].sys, m) || rtk->ssat[i].fix[f] != 2) {
				continue;
			}
			index[n[m]++] = IB(i + 1, f, &rtk->opt);
		}
		if (n[m] < 2) continue;
		for (i = 0;i < rtk->opt.sizes[m == 0 ? 0 : 1];i++) {
			if (!isValueInset(rtk->opt.satlist[m == 0 ? 0 : 1][i], index, n[m])) {
				flag = 0;
			}
		}
		{
	...
	/*GPS+BDS(all), GPS+BDS(separate)*/
	double* Test = mat(1, 1), * TestG = mat(1, 1), * TestC = mat(1, 1);
	double* tmpmatrix = mat(1, fixsatnum), * solarray = mat(1, fixsatnum), * Qsol = mat(fixsatnum, fixsatnum);
	if (flag && rtk->nfix >= 10) {
		int idxsat = 0, idxvari = 0, idxvariG = 0, idxvariC = 0;
		for (m = 0; m < 5; m++) {
			if (m == 1 || m == 2 || m == 4)continue;/*0 3 only for GPS and BDS*/
			double lam = m == 0 ? CLIGHT / FREQL1 : CLIGHT / FREQ1_CMP;/*默认单频L1和B1*/

			for (i = 0;i < rtk->opt.sizes[m == 0 ? 0 : 1];i++) {
				int iidex = rtk->opt.satlist[m == 0 ? 0 : 1][i];
				solarray[idxsat++] = lam * (xa[iidex] - rtk->x[iidex]);/*fix-float unit meter*/
			}
		}
		int num1 = rtk->opt.sizes[0];
		for (i = 0;i < fixsatnum; i++) {
			int iidex = rtk->opt.satlist[i < num1 ? 0 : 1][i < num1 ? i : i - num1];
			for (int j = 0;j < fixsatnum;j++) {
				int iidex1 = rtk->opt.satlist[j < num1 ? 0 : 1][j < num1 ? j : j - num1];
				double tmpvar = rtk->P[iidex * rtk->nx + iidex1];
				Qsol[idxvari++] = tmpvar;
			}
		}

		if (!(info = matinv(Qsol, fixsatnum))) {
			matmul("TN", 1, fixsatnum, fixsatnum, 1.0, solarray, Qsol, 0.0, tmpmatrix);/* Test=sol'*Q-1*sol */
			matmul("NN", 1, 1, fixsatnum, 1.0, tmpmatrix, solarray, 0.0, Test);
		}
		rtk->sol.test_sd = Test[0];
	}
	else {
		rtk->sol.test_sd = 0;
	}
	free(Test);free(tmpmatrix);free(solarray);free(Qsol);
}
...
static void holdamb(rtk_t* rtk, const double* xa)
{
	...
	if ((info = filter(rtk->x, rtk->P, H, v, R, rtk->nx, nv, rtk))) {
...
static int resamb_LAMBDA(rtk_t* rtk, double* bias, double* xa)
{
	...
	trace(3, "posvar=%.6f\n", var);
	/*上一历元固定解*/
	if (rtk->nfix > 0 && var > rtk->opt.thresar[1]) {
	...
	if (rtk->sol.time.time == 1614596417)
	{
		double a = 1;
	}
	...
	if (!(info = lambda(nb, 2, y + na, Qb, b, s))) {
		...
		if (s[0] <= 0.0 || s[1] / s[0] >= rtk->sol.thres) {
			...
			/*.......*/
			/*for float ambiguity Statistic Test 20241221*/
			int m, f, num[5], index[MAXSAT], flag = 1, nf = NF(&rtk->opt), iidex[MAXSAT], ddnum = 0, ddnumG = 0;
			/*Find the selected satellite from the fixed satellites*/
			for (m = 0; m < 5; m++) for (f = 0; f < nf; f++) {

				for (num[m] = i = 0; i < MAXSAT; i++) {
					if (!test_sys(rtk->ssat[i].sys, m) || rtk->ssat[i].fix[f] != 2) {
						continue;
					}
					index[num[m]++] = IB(i + 1, f, &rtk->opt);
				}
				if (num[m] < 2) continue;
				for (i = 0;i < rtk->opt.sizes[m == 0 ? 0 : 1];i++) {
					int tmpidex = rtk->opt.satlist[m == 0 ? 0 : 1][i];
					if (!isValueInset(tmpidex, index, num[m])) {
						flag = 0;
					}
					else if (tmpidex != index[0]) {
						for (j = 1;j < num[m];j++) {
							if (tmpidex == index[j]) {
								if (m == 0) ddnumG++;/*number of GPS dd ambiguity satellite*/
								{
									int didx = j - 1 + (m > 0 ? num[0] - 1 : 0);
									if (0 <= didx && didx < nb && ddnum < MAXSAT) {
										iidex[ddnum++] = didx;/*obtain the index of dd bias in y matrix*/
									}
									else flag = 0;
								}
								break;
...
#ifdef ENABLE_RTK_INTEGRITY
			if (rtk->int_child) flag = 0;
#endif
			if (flag && rtk->nfix >= 10 && ddnum > 0) {
				double* Test = mat(1, 1), * tmpmatrix = mat(1, ddnum);
				double* solarray = mat(1, ddnum), * Qsol = mat(ddnum, ddnum);
				int num1 = 0, num2 = 0;
				for (i = 0;i < ddnum;i++) {
					double lam = i < ddnumG ? CLIGHT / FREQL1 : CLIGHT / FREQ1_CMP;/*默认单频L1和B1*/
					solarray[num1++] = y[iidex[i] + na] * lam;/*dd ambiguity residual (ref to fixed)*/
					for (j = 0;j < ddnum;j++) {
						Qsol[num2++] = Qb[iidex[i] + iidex[j] * nb];
					}
				}
				if (!(info = matinv(Qsol, ddnum))) {
					matmul("TN", 1, ddnum, ddnum, 1.0, solarray, Qsol, 0.0, tmpmatrix);/* Test=sol'*Q-1*sol */
					matmul("NN", 1, 1, ddnum, 1.0, tmpmatrix, solarray, 0.0, Test);
				}
				rtk->sol.test_dd = Test[0];
				free(solarray);free(tmpmatrix);
				free(Qsol);free(Test);
			}
			else rtk->sol.test_dd = 0.0;
			/*for float ambiguity Statistic Test 20241221*/
...
{
	int i, f, lockc[NFREQ], ar = 0, excflag = 0, arsats[MAXOBS] = { 0 }, temp, locksave[MAXSAT] = { 0 }, lockflag[MAXSAT] = { 0 };
	int gps1 = -1, glo1 = -1, gps2, glo2, nb, rerun, dly = 1;
	float ratio1;
	double var_ar = 0;
	...
	//			arsats[ar++] = i;
	//		}
	//		else if (!rtk->ssat[sat[i] - 1].vsat[f] && rtk->ssat[sat[i] - 1].lock[f] >= 0) {
	//			rtk->ssat[sat[i] - 1].lock[f] = -1;
	//		}
	//	}
	//	/*sort by elevation*/
	//	for (int nn = 0; nn < ar - 1; nn++) {
	//		for (int mm = 0; mm < ar - nn - 1; mm++) {
	//			if (rtk->ssat[sat[arsats[mm]] - 1].azel[1] > rtk->ssat[sat[arsats[mm + 1]] - 1].azel[1]) {
	//				temp = arsats[mm]; arsats[mm] = arsats[mm + 1]; arsats[mm + 1] = temp;
	//			}
	//		}
	//	}
	//	if (rtk->excsat < 3) {
	...
	if (/*rtk->holdamb*/1) {
		...
		if (nb > 0) {
			var_ar = 0.0;
			for (i = 0; i < 3; i++) var_ar += rtk->Pa[i + i * rtk->na];
			var_ar = SQRT(var_ar / 3.0);
			if (var_ar > 0.04) {
				nb = 0;
			}
		}
		...
		if (rtk->opt.arfilter) {
			...
			if (nb >= 0 && rtk->sol.prev_ratio2 > 0 && ((rtk->sol.ratio < rtk->sol.thres) ||
				...
				for (i = 0; i < ns; i++) for (f = 0; f < nf; f++) {
					...
					if (rtk->ssat[sat[i] - 1].lock[f] == 0) {
						...
						//locksave[i] = rtk->ssat[sat[i] - 1].lock[f]; lockflag[i] = 1;
						rtk->ssat[sat[i] - 1].lock[f] = -rtk->opt.minlock - dly;  /* delay use of this sat with stagger */
						dly += 1;  /* stagger next try of new sats */
						rerun = 1;
					}
				}
			}
...
#if 1
			if (rerun == 1 && nb == 0) {/*扩大范围*/
				rerun = 0; dly = (dly > 2) ? dly : 2;
				for (i = 0; i < ns; i++) {
					f = 0;
					if (rtk->ssat[sat[i] - 1].fix[f] != 2) continue;
					if (rtk->ssat[sat[i] - 1].azel[1] < 15 * D2R &&
						((rtk->ssat[sat[i] - 1].half[f] & 1) || (rtk->ssat[sat[i] - 1].slip[1] & 2) || rtk->ssat[sat[i] - 1].slipc[0] >= 2)) {
						locksave[i] = rtk->ssat[sat[i] - 1].lock[f]; lockflag[i] = 1;
						rtk->ssat[sat[i] - 1].lock[f] = -rtk->opt.minlock - dly;  /* delay use of this sat with stagger */
						dly += 1;  /* stagger next try of new sats */
						rerun = 1;
				...
				if (nb == 0) {/*仍然未固定，则恢复lock和fix*/
					for (i = 0; i < ns; i++) {
						if (!lockflag[i]) continue;
						rtk->ssat[sat[i] - 1].fix[0] = 2;/*恢复fix*/
						rtk->ssat[sat[i] - 1].lock[0] = locksave[i];/*恢复lock*/
					}
				}
			}
#endif
	...
	var_ar = SQRT(var_ar / 3.0);
	if (var_ar > 0.04) {
		rtk->sol.ratio = rtk->sol.thres - 0.1;
		nb = 0;
	}
#endif
	...
	const nav_t* nav)
{
	...
	/*for shakedown test*/
	if (obs[nu - 1].time.time == gpst2time(2147, 127788).time) {
		int aaa = 8;
	}
	...
	for (i = 0; i < MAXSAT; i++) {
		...
		for (j = 0; j < NFREQ; j++) {
			...
			rtk->ssat[i].pld_rover[j] = 0;
			rtk->ssat[i].rv_rover[j] = 0;
			rtk->ssat[i].cv_rover[j] = 0;
		}
	}
	...
	for (i = 0; i < ns; i++) for (j = 0; j < nf; j++) {
		...
		/* PLD of base and rover receiver */
		rtk->ssat[sat[i] - 1].pld_rover[j] = obs[iu[i]].PLD[j];
		rtk->ssat[sat[i] - 1].rv_rover[j] = obs[iu[i]].R_V[j];/*伪距方差*/
		rtk->ssat[sat[i] - 1].cv_rover[j] = obs[iu[i]].C_V[j];/*相位方差*/
	}

	/* initialize Pp,xa to zero, xp to rtk->x */
	xp = mat(rtk->nx, 1); Pp = zeros(rtk->nx, rtk->nx); xa = mat(rtk->nx, 1);
	double* Pp0 = zeros(rtk->nx, rtk->nx);

	matcpy(xp, rtk->x, rtk->nx, 1);
	matcpy(Pp0, rtk->P, rtk->nx, rtk->nx);
...
#ifdef Open_Robust
		if (rtk->nx > 0) { /* robust algorithm after first ar*/
			info = robust_KF2(rtk, xp, Pp, v, H, R, rtk->nx, nv);
		}
#endif

		/* 标准卡尔曼滤波 */
		if ((info = filter(xp, Pp, H, v, R, rtk->nx, nv, rtk))) {
			errmsg(rtk, "filter error (info=%d)\n", info);
			stat = SOLQ_NONE;
			break;
		}
	...
	if (stat != SOLQ_NONE && zdres(0, obs, nu, rs, dts, var, svh, nav, xp, opt, 0, y, e, azel)) {
		...
		if (valpos(rtk, v, R, vflg, nv, 4.0, "float")) {
			...
			rtk->sol.ns = 0;
			for (i = 0; i < ns; i++) for (f = 0; f < nf; f++) {
				if (!rtk->ssat[sat[i] - 1].vsat[f]) continue;/*未参与滤波的卫星outc++*/
	...
	for (i = 0; i < MAXSAT; i++) for (j = 0; j < nf; j++) {
		...
		/*该颗卫星没有参与模糊度固定*/
		if (rtk->ssat[i].fix[j] < 2 && rtk->ssat[i].lock[j] > 0) {
			rtk->ssat[i].lock[j] = 0;
		}
	}
	free(rs); free(dts); free(var); free(y); free(e); free(azel);
	free(xp); free(Pp); free(Pp0); free(xa);  free(v); free(H); free(R); free(bias);
...
*-----------------------------------------------------------------------------*/
extern int rtkpos(rtk_t* rtk, const obsd_t* obs, int n, const nav_t* nav, const sta_t* stalin)
{
	prcopt_t* opt = &rtk->opt;
	sol_t solb = { {0} };
	gtime_t time;
	...
	/*Initialize  rover coordinates*/
	if (rtk->sol.rr[0] == 0.0) {/*first epoch*/
		rtk->sol.rr[0] = stalin[0].pos[0];
		rtk->sol.rr[1] = stalin[0].pos[1];
		rtk->sol.rr[2] = stalin[0].pos[2];
	}
