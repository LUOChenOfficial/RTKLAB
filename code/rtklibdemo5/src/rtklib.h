...
//#ifndef Open_Robust
//#define Open_Robust    /*Open robust*/
//#endif // Open_Robust
...
#define WEIGHTOPT_PLD       2           /* weighting option: pld */
#define WEIGHTOPT_ALL       3           /* weighting option: all */
#define WEIGHTOPT_ELE_SNR   4           /* weighting option: ele+snr */
#define WEIGHTOPT_ELE_PLD   5           /* weighting option: ele+pld */
#define WEIGHTOPT_SNR_PLD   6           /* weighting option: snr+pld */
#define WEIGHTOPT_CV_PV     7           /* weighting option: code+phase variance */
#define WEIGHTOPT_ALL_CPV   8           /* weighting option: all+code+phase variance */
...
#define NX          (4+3)       /* # of estimated parameters-PH-20211229 */
...
typedef struct {        /* observation data record */
    ...
    //读入PLD、载波卡尔曼方差、伪距卡尔曼方差
    double PLD[NFREQ + NEXOBS]; /* observation PLD */
    double R_V[NFREQ + NEXOBS]; /* Range variance */
    double C_V[NFREQ + NEXOBS]; /* Carrier phase variance */
} obsd_t;
...
typedef struct {        /* solution type */
    ...
    int GPS_NUM;        /*GPS satellite number*/
    int BDS_NUM;        /*BDS satellite number*/
    int GLO_NUM;        /*GLONASS satellite number*/
    int GAL_NUM;        /*GALILEO satellite number*/
...
typedef struct {        /* processing options type */
    ...
    int *satlist[2];/* GPS and BDS fixed satellites list*/
    int sizes[2];/* satellites number of each sys*/
#ifdef ENABLE_RTK_INTEGRITY
    int enable_rtk_integrity_monitor;
    int enable_rtk_integrity_rbias_export;
    int enable_monitor_single_satellite_fault;
    int enable_monitor_fixed_ambiguity_update_fault;
    int enable_rtk_integrity_fde_recovery;
    int rtk_integrity_max_subset_filters;
    int enable_rtk_integrity_subset_debug_output;
...
typedef struct {        /* satellite status type */
    ...
    unsigned char pld_rover[NFREQ]; /* ph-20211104 */
    //unsigned char pld_base[NFREQ]; /* ph-20211104 */
    double rv_rover[NFREQ];/*range var*/
    double cv_rover[NFREQ];/*carrier phase var*/
    ...
    double  cmc;        /* code minus carrier phase(m) */
    double  deltcmc;    /*delt cmc*/
...
#ifdef ENABLE_RTK_INTEGRITY
#define RTKLIB_IN_RTK_INT
#include "rtk_int.h"
#undef RTKLIB_IN_RTK_INT
#endif

typedef struct rtk_tag {        /* RTK control/result type */
...
EXPORT double getmattr(double* A, int n); // 求矩阵的秩-20211229-PH
...
EXPORT int  lsq   (const double *A, const double *y, int n, int m, double *x,
                   double *Q);
EXPORT int Helmertest(int nv, int ns, double* Q, double* v, double* H,
                   double* dx, sol_t* sol); //方差分量估计函数声明-20211229-PH
