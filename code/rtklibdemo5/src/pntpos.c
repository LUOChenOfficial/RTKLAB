...
static double varerr(const prcopt_t *opt, double el, int prn, int sys)
{
    double a, b;
...
/*/static int rescode(int iter, const obsd_t* obs, int n, const double* rs,
                   ...
                   double *azel, int *vsat, double *resp, int *ns, sol_t *sol)*///新加sol_t* sol，PH-20211229
static int rescode(int iter, const obsd_t* obs, int n, const double* rs,
	const double* dts, const double* vare, const int* svh,
	const nav_t* nav, const double* x, const prcopt_t* opt,
	const ssat_t* ssat, double* v, double* H, double* var,
	double* azel, int* vsat, double* resp, int* ns)
{
	...
	for (i = *ns = 0; i < n && i < MAXOBS; i++) {
		...
		/*if (sys == SYS_GPS) { sol->GPS_NUM++; }
		else if (sys == SYS_QZS) { sol->QZS_NUM++; }
		else if (sys == SYS_GLO) { v[nv] -= x[4]; H[4 + nv * NX] = 1.0; mask[1] = 1; sol->GLO_NUM++; }
		else if (sys == SYS_GAL) { v[nv] -= x[5]; H[5 + nv * NX] = 1.0; mask[2] = 1; sol->GAL_NUM++; }
		else if (sys == SYS_CMP) { v[nv] -= x[6]; H[6 + nv * NX] = 1.0; mask[3] = 1; sol->BDS_NUM++; }
		else mask[0] = 1;*///PH-20220106
		...
		/* error variance */
		var[nv++] = varerr(opt, azel[1 + i * 2], prn, sys) + vare[i] + vion + vtrp;//+ vmeas
                  ...
                  char *msg,sol_t* sol)
{
	...
	memcpy(sol->dops, dop, sizeof(double) * 4);
    return 1;
}
